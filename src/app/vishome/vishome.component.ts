...
@Component({
  selector: 'app-vishome',
  templateUrl: './vishome.component.html',
  styleUrls: ['./vishome.component.css'],

})
export class VishomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.fileName=fName!=null?fName:"";
    const chapter = this.route.snapshot.paramMap.get('chapter');
    this.chapName=chapter!=null?chapter:"";
    console.log(fName);
    var query="select * from JoinedSchema where filename='"+fName+"' and chaptername='"+chapter+"'";
    const httpOptions = {
